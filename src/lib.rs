...
pub struct VecWithPosition<Active: ActiveResource, Inactive> {
    inactive: Vec<Inactive>,
    active: Option<Position>,
}

impl<Active: ActiveResource, Inactive> Default for VecWithPosition<Active, Inactive> {
    fn default() -> Self {
        Self::new()
    }
}
...
impl<'a, Active: ActiveResource, Inactive: Clone> ResourcePool<Active, Inactive> {
    ...
    pub async fn remove(&mut self) -> Option<Active> { // TODO: Duplicate code.
        if let Some(ref active) = self.active {
            let result = self.next(); // self.reallocate_position(active).await;
            self.inactive.remove(active.position().0);
            result
        } else {
            None
        }
